...
const WechatDataTool = require('../utils/WechatDataTool')
...
const WechatLoginService = {
    ...
    login: async () => {
        ...
        redis.set(key, JSON.stringify({ isScan: 'no' }), 120)
    ...
    wechat_message: async (req) => {
        ...
        // 正式版本
        // let xmlData = await WechatDataTool.getXMLStr(req)
        let objectData = await WechatDataTool.getObject(xmlData)
        let lastData = WechatDataTool.getLastData(objectData.xml)
        ...
        const existsKey = await redis.exists(key)
        if (existsKey) {
            redis.set(key, JSON.stringify({ isScan: 'yes', token }), 120)
        }
    ...
    check_scan: async (req) => {
        ...
        let key = `wechat:ticket:${ticket}`
        let redisData = JSON.parse(await redis.get(key))
...
module.exports = WechatLoginService